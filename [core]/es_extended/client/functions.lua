...
function ESX.Game.CopyToClipboard(dataType)
    ...
    if dataType == 'coords2' then
        message = string.format('vector2(%s, %s)', x, y)
        notificationMessage = "Copied vector2"
    elseif dataType == 'coords3' then
        message = string.format('vector3(%s, %s, %s)', x, y, z)
        notificationMessage = "Copied vector3"
    elseif dataType == 'coords4' then
        message = string.format('vector4(%s, %s, %s, %s)', x, y, z, h)
        notificationMessage = "Copied vector4"
    elseif dataType == 'heading' then
        message = h
        notificationMessage = "Copied heading"
    elseif dataType == 'freeaimEntity' then
        ...
        if entity then
            ...
            message = string.format('Model Name:\t%s\nModel Hash:\t%s\n\nHeading:\t%s\nCoords:\t\tvector3(%s, %s, %s)\nRotation:\tvector3(%s, %s, %s)', entityName, entityHash, h, x, y, z, rotX, rotY, rotZ)
            notificationMessage = "Copied freeaim entity"
        else
            notificationType = "error"
            notificationMessage = "Error: Ped not exist"
        end
    end
    ...
    ESX.ShowNotification(notificationMessage, notificationType)
end
...
Citizen.CreateThread(function()
    RegisterFontFile('arial') ESX.Font = RegisterFontId('arial font')
