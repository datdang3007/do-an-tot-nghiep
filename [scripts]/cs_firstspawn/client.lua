...
RegisterNetEvent('cs:option:setOption', function(option)
    if option == 1 then
        targetCoords = CodeStudio.SafeTaxiDestination
        nearestTargetCoords = CodeStudio.SafeSkipToNearestLoc
    elseif option == 2 then
        targetCoords = CodeStudio.DangerousTaxiDestination
        nearestTargetCoords = CodeStudio.DangerousSkipToNearestLoc
    end
...
RegisterNetEvent('cs:introCinematic:start', function()
    ...
    TriggerEvent('cuoi-ui:nui:toggle', false, 'chose the path')
end)
		...
		if IsThisModelACar(taxiModel) then
			...
			if not DoesEntityExist(taxiVeh) then
                ...
                if IsPedInVehicle(PlayerPedId(), taxiVeh, 1) then
                    ...
                    TaskVehicleDriveToCoord(taxiPed, taxiVeh, targetCoords.x, targetCoords.y, targetCoords.z, targetCoords.w, 0, GetEntityModel(taxiVeh), 524604, 10.0)
...
Citizen.CreateThread(function()
    ...
    while true do
        if Active then
            local dist = #(GetEntityCoords(PlayerPedId()) - vector3(targetCoords.x, targetCoords.y, targetCoords.z))
            if dist <= 10 then
            ...
            if isTaxi then
                ...
                if IsControlJustPressed(1, 194) then
                    ...
                    Wait(3000)
                    SetEntityCoords(vehicle, vector3(nearestTargetCoords.x, nearestTargetCoords.y, nearestTargetCoords.z))
                    SetEntityHeading(vehicle, nearestTargetCoords.w)
                    DoScreenFadeIn(250)
                end
            end
        end
