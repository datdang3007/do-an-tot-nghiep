	...
	while true do
		...
		for k = 1, #Config.PedList, 1 do
			...
			if dist < Config.Distance and not v.isRendered then
				local ped = nearPed(v.model, v.coords, v.heading, v.gender, v.animDict, v.animName, v.scenario, v.gun)
				v.ped = ped
				v.isRendered = true
			end
...
function nearPed(model, coords, heading, gender, animDict, animName, scenario, gun)
	local genderNum = 0
	...
	if gun then
		GiveWeaponToPed(ped, gun, 0, false, true)
		SetCurrentPedWeapon(ped, gun, true)
		SetAmmoInClip(ped, gun, 255)
